...
import { performTimeCapsuleDivination } from '@/algorithms/timeCapsule'
import { saveTimeCapsuleResult, getTimeCapsuleRecordById, getAllTimeCapsuleRecords, updateTimeCapsuleResult } from '@/utils/storage'
import { DeterministicRandom } from '@/utils/math'
import type { TimeCapsuleInput, TimeCapsuleResult, TimeCapsuleRealityCheck, RitualPhase, Rune, StarPoint } from '@/types'
...
export const useTimeCapsuleStore = defineStore('timeCapsule', () => {
  ...
  const currentResult = ref<TimeCapsuleResult | null>(null)
  ...
  const canProceedToRunes = computed(() => {
    return name.value.trim().length > 0 && 
           ...
           targetDate.value.length > 0 &&
           new Date(targetDate.value) > new Date()
  })
  ...
  const initStarPoints = (width: number, height: number) => {
    const seed = `${name.value}-${birthDate.value}-${targetDate.value}-${divinationTimestamp.value}`
  ...
  const startCalculation = async () => {
    ...
    const result = performTimeCapsuleDivination(input)
    calculationProgress.value = 100
    
    await new Promise(resolve => setTimeout(resolve, 300))
    
    currentResult.value = result
    saveTimeCapsuleResult(result)
    phase.value = 'complete'
    isCalculating.value = false
  }

  const loadResult = (id: string): boolean => {
    const result = getTimeCapsuleRecordById(id)
    if (result) {
      currentResult.value = result
      return true
    }
  ...
  const unlockCapsule = (id: string): boolean => {
    const result = getTimeCapsuleRecordById(id)
    if (result && Date.now() >= result.unlockAt && !result.isUnlocked) {
      result.isUnlocked = true
      updateTimeCapsuleResult(result)
      currentResult.value = result
      return true
    }
    return false
  ...
  const submitRealityCheck = (id: string, realityCheck: TimeCapsuleRealityCheck): boolean => {
    const result = getTimeCapsuleRecordById(id)
    if (result && result.isUnlocked) {
      result.realityCheck = realityCheck
      updateTimeCapsuleResult(result)
      currentResult.value = result
      return true
    }
  ...
  const getAvailableCapsules = () => {
    return getAllTimeCapsuleRecords().filter(c => !c.isUnlocked && Date.now() >= c.unlockAt)
  }

  const getLockedCapsules = () => {
    return getAllTimeCapsuleRecords().filter(c => !c.isUnlocked && Date.now() < c.unlockAt)
  }

  const getCompletedCapsules = () => {
    return getAllTimeCapsuleRecords().filter(c => c.isUnlocked)
  }
